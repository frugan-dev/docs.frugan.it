...
La webmail per accedere alle caselle email `Google Workspace <https://workspace.google.com>`_ è raggiungibile al seguente indirizzo:

- `https://mail.domain.tld` (GMail)
...
I parametri necessari per l'autenticazione ai server di posta `Google Workspace <https://workspace.google.com>`_ tramite i comuni client di posta elettronica (ad es. Outlook, Thunderbird, ecc.) sono uguali per tutte le caselle ospitate presso `Frugan <https://frugan.it>`_:
...
.. 
	...
	admonition (non visibile)
	title (diventa il titolo della pagina)
.. note:: A prescindere dal dominio utilizzato, l'autenticazione ai server di posta `Google Workspace <https://workspace.google.com>`_ avviene sempre tramite hostname del tipo `*.gmail.com` o `*.googlemail.com`.
...
Workspace ha un unico contenitore in cui finiscono tutti i messaggi, compresa la posta inviata che viene salvata in automatico.
...
- https://workspace.google.com/intl/it/support/